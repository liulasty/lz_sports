      ...
      <div v-else>
        ...
        <div v-else class="status-box">
          <el-alert
            ...
            class="status-alert"
          />
          <div class="tips" v-if="applicationStatus === '在审核' || applicationStatus === 'PENDING'">
            您的申请正在由管理员审核中，请耐心等待。审核结果将通过站内信通知您。
          </div>
          <div class="tips" v-else-if="applicationStatus === '成功' || applicationStatus === 'ACTIVE'">
...
const getStatusType = (status) => {
  if (status === '成功' || status === 'ACTIVE') return 'success'
  if (status === '在审核' || status === 'PENDING') return 'warning'
  return 'error'
}
